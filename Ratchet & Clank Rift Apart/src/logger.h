...
inline const wchar_t* LogPath(){
    ...
    if(GetModuleFileNameW(SelfModule(),c[n],MAX_PATH)){ if(wchar_t*s=wcsrchr(c[n],L'\\')){*(s+1)=0; wcscat_s(c[n],MAX_PATH,L"HeadTracking.log"); n++; } }
    if(GetModuleFileNameW(nullptr,c[n],MAX_PATH)){ if(wchar_t*s=wcsrchr(c[n],L'\\')){*(s+1)=0; wcscat_s(c[n],MAX_PATH,L"HeadTracking.log"); n++; } }
    { wchar_t t[MAX_PATH]; if(GetTempPathW(MAX_PATH,t)){ wcscpy_s(c[n],MAX_PATH,t); wcscat_s(c[n],MAX_PATH,L"HeadTracking.log"); n++; } }
    { if(GetEnvironmentVariableW(L"USERPROFILE",c[n],MAX_PATH)){ wcscat_s(c[n],MAX_PATH,L"\\Desktop\\HeadTracking.log"); n++; } }
    wcscpy_s(c[n],MAX_PATH,L"C:\\HeadTracking.log"); n++;
    for(int i=0;i<n;i++){
        ...
        if(h!=INVALID_HANDLE_VALUE){ CloseHandle(h); wcscpy_s(chosen,MAX_PATH,c[i]);
            char m[MAX_PATH]; wcstombs(m,chosen,MAX_PATH); OutputDebugStringA("[HeadTracking] log -> "); OutputDebugStringA(m); OutputDebugStringA("\n"); return chosen; }
    }
    return nullptr;
}
...
inline void Log(const char* fmt, ...) {
    ...
    OutputDebugStringA("[HeadTracking] ");
