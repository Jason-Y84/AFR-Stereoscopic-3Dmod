...
static HRESULT STDMETHODCALLTYPE hkPresent(IDXGISwapChain* sc, UINT sync, UINT flags) {
    ...
    //Scan_Poll();  // disabled: AOB-based cutscene hook replaces this scan
    PaceFrame();
...
// Forward declaration for cutscene camera scanner
void ScanForCutsceneCamera();
...
void  Stereo_PresentEyeFallback() {
    ...
    if (presentsNow - lastCheck >= 10) {
        ...
        if (delta == 0 && presentsNow > 30) {
            g_fallbackActive.store(1, std::memory_order_relaxed);
            // (state-change log removed for release)
        } else if (delta > 0) {
            g_fallbackActive.store(0, std::memory_order_relaxed);
        }
    }
...
// ------------------------------------------------------------------ cutscene scan ----
// Scan for camera-like structures in process memory during cutscenes.
// When the main camera hook goes silent, the game may switch to a different
// rendering path with a different camera structure. This scan helps identify
// those addresses by finding memory regions that match the camera layout
// (pos[3] + lookAt[3] + up[3] as contiguous floats at known offsets).
namespace {
...
static uint8_t*   s_scanCandidates[256] = {};
static int        s_scanCandidateCount = 0;
static bool       s_scanRunning = false;
static int        s_scanPhase = 0;   // 0=first scan, 1=waiting, 2=second scan, 3=done
static float      s_scanSnapshots[256][3] = {};  // store first snapshot of pos[3]

// Check if a float array looks like a valid position (not NaN, not infinity,
// and within reasonable game coordinates range)
bool IsValidCamFloat(float v) {
    if (v != v) return false;  // NaN check
    if (v > 1e8f || v < -1e8f) return false;
    if (v == 0.0f) return true;
    // Check for very large integer patterns (likely not game coords)
    float av = fabsf(v);
    if (av > 1e6f && v == (float)(int)v) return false;
    return true;
}

bool IsValidCamPos(const float* pos) {
    return IsValidCamFloat(pos[0]) && IsValidCamFloat(pos[1]) && IsValidCamFloat(pos[2]);
}

bool IsStaticPlaceholder(const float* pos) {
    // Known cutscene placeholder: [8, 8, 22]
    return (pos[0] > 7.9f && pos[0] < 8.1f &&
            pos[1] > 7.9f && pos[1] < 8.1f &&
            pos[2] > 21.9f && pos[2] < 22.1f);
}

void FindCandidatesInRange(uint8_t* base, size_t size, CamCandidate* out, int& outCount) {
    // Scan for structures matching the camera layout pattern:
    //   offset +0x00: unknown / padding
    //   offset +0x10: pos[3] (3 floats)
    //   offset +0x20: lookAt[3] (3 floats)
    //   offset +0x30: up[3] (3 floats)
    // We check pos[3] at +0x10, tgt[3] at +0x20, up[3] at +0x30
    if (size < 0x60) return;
    ...
    uint8_t* end = base + size - 0x40;

    while (p < end && outCount < 256) {
        float* pos = (float*)(p + 0x10);
        float* tgt = (float*)(p + 0x20);
        float* up  = (float*)(p + 0x30);

        // Quick check: all 9 floats must be valid
        if (!IsValidCamPos(pos) || !IsValidCamPos(tgt) || !IsValidCamPos(up)) {
            p += 8;
            continue;
        }

        // Skip known static placeholders
        if (IsStaticPlaceholder(pos)) {
            p += 8;
            continue;
        }

        // Check that pos, tgt, up are reasonably different (not all identical)
        float dpos = fabsf(pos[0]-pos[1]) + fabsf(pos[1]-pos[2]);
        float dtgt = fabsf(tgt[0]-tgt[1]) + fabsf(tgt[1]-tgt[2]);
        if (dpos < 0.001f && dtgt < 0.001f) {
            p += 8;
            continue;
        }

        // Check up vector: should be near unit length
        float upLen = sqrtf(up[0]*up[0] + up[1]*up[1] + up[2]*up[2]);
        if (upLen < 0.5f || upLen > 1.5f) {
            p += 8;
            continue;
        }

        // Check distance: |tgt - pos| should be reasonable (0.1 to 10000)
        float dx = tgt[0]-pos[0], dy = tgt[1]-pos[1], dz = tgt[2]-pos[2];
        float dist = sqrtf(dx*dx + dy*dy + dz*dz);
        if (dist < 0.1f || dist > 10000.0f) {
            p += 8;
            continue;
        }

        // This looks like a camera structure
        s_scanCandidates[outCount] = p;
        outCount++;
        p += 0x10;  // Skip past one potential structure, move to next
    }
}

void DoMemoryScan() {
    s_scanCandidateCount = 0;
    ...
    while (VirtualQuery(addr, &mbi, sizeof(mbi)) == sizeof(mbi)) {
        if (mbi.State == MEM_COMMIT &&
            ...
            // Don't scan module code pages - focus on heap/stacks
            if (!(mbi.Protect & PAGE_EXECUTE)) {
                FindCandidatesInRange((uint8_t*)mbi.BaseAddress, mbi.RegionSize,
                                       nullptr, s_scanCandidateCount);
            }
        }
        addr = (uint8_t*)mbi.BaseAddress + mbi.RegionSize;
        if (addr <= (uint8_t*)mbi.BaseAddress) break;  // overflow
    }
}
...
void ScanForCutsceneCamera() {
    ...
    s_scanRunning = true;
    s_scanPhase = 0;
    Log("[Scan] Starting cutscene camera memory scan...");

    // Phase 1: First scan - find candidates
    DoMemoryScan();
    Log("[Scan] Phase 1 complete: found %d candidate camera structures", s_scanCandidateCount);

    // Save first snapshot
    for (int i = 0; i < s_scanCandidateCount; i++) {
        float* pos = (float*)(s_scanCandidates[i] + 0x10);
        memcpy(s_scanSnapshots[i], pos, 12);
        Log("[Scan] Candidate[%d] addr=%p pos=[%.2f %.2f %.2f]",
            i, s_scanCandidates[i], pos[0], pos[1], pos[2]);
    }

    s_scanPhase = 1;
}

// Called periodically to check for changes in candidate structures
void Scan_Poll() {
    if (s_scanPhase == 1) {
        // Wait a bit, then do second scan to see which addresses changed
        static int s_waitFrames = 0;
        if (++s_waitFrames < 60) return;  // Wait ~1 second (60 frames)
        s_waitFrames = 0;
        s_scanPhase = 2;

        Log("[Scan] Phase 2: checking which candidates have changed...");
        int changedCount = 0;
        for (int i = 0; i < s_scanCandidateCount; i++) {
            if (!IsBadReadPtr(s_scanCandidates[i] + 0x10, 12)) {
                float* pos = (float*)(s_scanCandidates[i] + 0x10);
                float dx = pos[0] - s_scanSnapshots[i][0];
                float dy = pos[1] - s_scanSnapshots[i][1];
                float dz = pos[2] - s_scanSnapshots[i][2];
                float d2 = dx*dx + dy*dy + dz*dz;
                if (d2 > 0.001f) {
                    changedCount++;
                    Log("[Scan] CHANGED[%d] addr=%p pos=[%.2f %.2f %.2f] dpos=[%.3f %.3f %.3f]",
                        i, s_scanCandidates[i],
                        pos[0], pos[1], pos[2], dx, dy, dz);
                }
            }
        }
        Log("[Scan] Phase 2 complete: %d/%d candidates changed", changedCount, s_scanCandidateCount);
        s_scanPhase = 3;
        s_scanRunning = false;
    }
}
